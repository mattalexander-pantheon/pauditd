...
# - COVERALLS_REPO_TOKEN: (optional) Token to use when pushing coverage to coveralls.
#
#-------------------------------------------------------------------------------
...
# Python dependencies
FLAKE8_BIN := $(shell which flake8 2> /dev/null)
PYLINT_BIN := $(shell which pylint 2> /dev/null)
COVERAGE_BIN := $(shell which coverage 2> /dev/null)
COVERALLS_BIN := $(shell which coveralls 2> /dev/null)
BUMPVERSION_BIN := $(shell which bumpversion 2> /dev/null)
...
ifeq (,$(shell which trial 2> /dev/null))
	pip install twisted
endif
endif
...
ifdef COVERALLS_REPO_TOKEN
	...
	echo "COVERALLS_REPO_TOKEN is not set. Skipping coveralls reporting"
endif
